...
def hostname_option(hostname=None):
    ...
    if hostname and len(hostname) > 64:
        utils.warn("Hostnames of many OSes are limited to 64 characters." f"The current hostname {hostname} is {len(hostname)}.")
...
def ports_option(ports=None):
    ...
    if type(ports) is not list:
        utils.abort(f"`ports` was not evaluated as a Python list, but as '{type(ports)}'.")
    for port_pair in ports:
        if type(port_pair) is not list:
            utils.abort(f"`ports` element {port_pair} was not evaluated as a Python list, but as " f"'{type(port_pair)}'.")
        if len(port_pair) != 2:
...
def size_option(ram_size=None, drive_size=None):
    ...
    if ram_size and not drive_size:
        ...
        try:
            ram_size = SETTINGS["SIZES"][list(SETTINGS["SIZES"].values()).index(drive_size)]
...
def sync_dirs_option(sync_dirs=None):
    ...
    if type(sync_dirs) is not list:
        utils.abort(f"sync_dirs was not evaluated as a Python list, but as '{type(sync_dirs)}'")
    for sd in sync_dirs:
        if type(sd) is not list:
            utils.abort(f"sync_dirs element {sd} was not evaluated as a Python list, but as " f"'{type(sd)}'")
...
def sync_type_option(sync_type=None):
    ...
    if sync_type in SETTINGS["SYNC_TYPES"]:
        ...
        utils.warn(
            f"Sync type {sync_type} not in approved list. Using Vagrant's default." f"Supported alternate sync types are {SETTINGS['SYNC_TYPES']}."
        )
        sync_type = None
