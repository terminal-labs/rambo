...
class ConfigFileProcessor(ConfigFileReader):
    ...
    config_section_primary_schemas = [ConfigSectionSchema.Base, ConfigSectionSchema.Up]
...
@cli.command("up", context_settings=CONTEXT_SETTINGS, short_help="Create or start VM.")
@click.option(
    "-p",
    "--provider",
    type=str,
...
@click.option(
    "--sync-dir", type=click.Path(resolve_path=True), help="Path to sync into VM"
)
@click.option(
...
def up_cmd(
    ...
    if not os.path.isfile("%s.conf" % PROJECT_NAME):
        utils.abort(
            "Config file %s.conf must be present in working directory.\n"
            "A config file is automatically created when you run \n"
            "createproject. You can also make a config file manually." % PROJECT_NAME
        )
